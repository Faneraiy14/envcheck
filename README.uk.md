...
PHPUnit, 14 тестів: повна відповідність .env/.env.example, відсутні/порожні/зайві
ключі, `--fix`/`--strict`/`--json` через реальний виклик процесу
(`tests/CliTest.php`), парсер (коментарі, `export`, лапки, UTF-8 BOM на
початку файлу), помилка на неіснуючому шляху (`tests/EnvCheckerTest.php`).
